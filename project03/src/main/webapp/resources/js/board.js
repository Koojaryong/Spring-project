...
function wrapCreateBoardByMask2() {
// 화면의 높이와 너비를 구한다.
var maskHeight = $(document).height();
var maskWidth = $(window).width();

// 마스크의 높이와 너비를 화면 것으로 만들어 전체 화면을 채운다.
$('#mask_board_manage').css({
'width' : maskWidth,
'height' : maskHeight
});

$('#mask_board_manage').fadeTo("slow", 0.7);

$('.normal_manage_board_modal').show();
}
...
//닫기 버튼을 눌렀을 때
$('.normal_manage_board_modal .top .close').click(function(e) {
//링크 기본동작은 작동하지 않도록 한다.
e.preventDefault();
$('#mask_board_manage, .normal_manage_board_modal').hide();
});

//검은 막을 눌렀을 때
$('#mask_board_manage').click(function() {
$(this).hide();
$('.normal_manage_board_modal').hide();
});
